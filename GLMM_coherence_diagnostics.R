...
#  S3 ENCODING + GAP HANDLING (3 interaction models): observed ar1 vs full-grid
#     ar1 vs ou; and numFactor vs rank-order equivalence -- completes VWC & flux.
#  S4 PER-YEAR MODEL-VS-RAW (3 interaction models).
#  S5 RESIDUAL AUTOCORR lag 1/7/14 (all final fits).
#  S6 MECHANISTIC QUANTITIES under new spec: Q10 by treatment, VWC optimum by
#     treatment (delta-method CIs), residual treatment effect -- to check whether
#     locked values (Q10 1.62/1.39; optima 0.30/0.20) survive + Year_f + season.
#
# Marginals/effects: EQUAL weights (emmeans default) to reconcile with locked values.
...
suppressPackageStartupMessages({
  ...
  library(glmmTMB); library(emmeans); library(numDeriv)
})
...
tvwc<-read_csv("T_VWC.csv",show_col_types=FALSE) %>%
  ...
  summarise(temp_mean=mean(Temperature,na.rm=TRUE),vwc_mean=mean(VWC,na.rm=TRUE),.groups="drop") %>%
  mutate(vwc_pct=vwc_mean*100,Year=year(Date),Year_f=factor(Year),season=season_of(Date),
         Plot_Year=factor(paste(Plot,Year,sep="_"))) %>% filter(!is.na(season)) %>% arrange(Date) %>% add_time()
flux<-read_csv("FLUX.csv",show_col_types=FALSE) %>%
  mutate(Date=as.Date(DayHour),Treatment=factor(Treatment,levels=c("control","warmed")),Plot=factor(Plot)) %>%
  filter(Date>=as.Date("2018-09-01"),!is.na(Flux),Flux>0) %>%
  group_by(Date,Plot,Treatment) %>%
  summarise(flux=mean(Flux),temp_mean=mean(Temperature,na.rm=TRUE),vwc_mean=mean(VWC,na.rm=TRUE),.groups="drop") %>%
...
get_phi<-function(m){vc<-VarCorr(m)$cond;cm<-attr(vc[[length(vc)]],"correlation")
  if(!is.null(cm)&&nrow(cm)>=2) cm[1,2] else NA_real_}
get_marg<-function(m){em<-emmeans::emmeans(m,~Treatment)
  mg<-as.data.frame(summary(emmeans::contrast(em,method="revpairwise"),infer=c(TRUE,TRUE)))
  c(est=mg$estimate[1],se=mg$SE[1])}
fit_capped<-function(expr){setTimeLimit(cpu=FIT_TIME_CAP,elapsed=FIT_TIME_CAP,transient=TRUE)
  on.exit(setTimeLimit(cpu=Inf,elapsed=Inf)); tryCatch(eval.parent(substitute(expr)),error=function(e) e)}
fitm<-function(rhs,data,time_term="ar1(time_num+0|Plot_Year)",start=NULL){
  f<-as.formula(paste(rhs,"+ (1|Plot) +",time_term))
  fit_capped(glmmTMB(f,dispformula=DISP,data=data,REML=TRUE,start=start,control=ctrl))}
...
for(nm in names(MODELS)){
  ...
  if(!isTRUE(m$sdr$pdHess)){
    ...
    if(!inherits(m1,"error") && isTRUE(m1$sdr$pdHess)){
      ...
      wr(sprintf("  %-16s [fallback disp~1] did not certify; retaining ~Treatment fit\n",nm))
    }
  }
}
...
for(nm in names(MODELS)){
  ...
  for(i in 1:5){ st<-list(beta=be0,betadisp=bd0,theta=th0+rnorm(np,0,0.5))
    m<-fitm(MODELS[[nm]]$rhs,MODELS[[nm]]$data,start=st); a<-addfit(i,m); if(!is.null(a)) res[[paste0("s",i)]]<-a }
...
encs<-c(observed_ar1="ar1(time_num+0|Plot_Year)",fullgrid_ar1="ar1(time_grid+0|Plot_Year)",
        ou_continuous="ou(time_num+0|Plot_Year)",rankorder_ar1="ar1(time_rank+0|Plot_Year)")
for(nm in c("temp_inter","vwc_inter","flux_inter")){
  wr("\n-- ",nm," --\n")
  for(en in names(encs)){
    m<-fitm(MODELS[[nm]]$rhs,MODELS[[nm]]$data,encs[en])
...
for(nm in c("temp_inter","vwc_inter","flux_inter")){
  ...
  raw<-data %>% group_by(Year_f,Treatment) %>% summarise(mu=mean(.data[[resp]],na.rm=TRUE),.groups="drop") %>%
    pivot_wider(names_from=Treatment,values_from=mu) %>% mutate(raw_diff=warmed-control)
  cmp<-mc %>% select(Year_f,model_diff=estimate) %>% left_join(raw %>% select(Year_f,raw_diff),by="Year_f") %>%
    mutate(discrepancy=model_diff-raw_diff)
  wr("\n-- ",nm," (max |disc|=",sprintf("%.4f",max(abs(cmp$discrepancy),na.rm=TRUE)),") --\n")
...
if(!is.null(mm)){
  ...
  wr("  [locked reference: Q10 1.62/1.39; optima 0.300/0.205 (concave). Compare;\n")
  wr("   temp/VWC here from FLUX.csv columns, not T_VWC.csv -- centring may differ.]\n")
...
wr("mechanistic quantities under new spec vs locked values.\n")
